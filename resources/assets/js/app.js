...
require('./pilot_tinymce');
window.pilotTinyMCE = require('./pilot_tinymce_config');
require('./pilot_ace_editor');
...
$(document).ready(function () {
    ...
    $('#pilot-loader').fadeOut();

    $(".hamburger, .navbar-expand-toggle").on('click', function () {
        appContainer.toggleClass("expanded");
        $(this).toggleClass('is-active');
        if ($(this).hasClass('is-active')) {
            window.localStorage.setItem('pilot.stickySidebar', true);
        } else {
            window.localStorage.setItem('pilot.stickySidebar', false);
        }
    });
    ...
    $(document).on('click', '.panel-heading a.panel-action[data-toggle="panel-collapse"]', function (e) {
        ...
        if (!$this.hasClass('panel-collapsed')) {
            ...
            $this.removeClass('pilot-angle-up').addClass('pilot-angle-down');
        } else {
            $this.parents('.panel').find('.panel-body').slideDown();
            $this.removeClass('panel-collapsed');
            $this.removeClass('pilot-angle-down').addClass('pilot-angle-up');
        }
    });
    ...
    $(document).on('click', '.panel-heading a.panel-action[data-toggle="panel-fullscreen"]', function (e) {
        e.preventDefault();
        var $this = $(this);
        if (!$this.hasClass('pilot-resize-full')) {
            $this.removeClass('pilot-resize-small').addClass('pilot-resize-full');
        } else {
            $this.removeClass('pilot-resize-full').addClass('pilot-resize-small');
        }
