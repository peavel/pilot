...
<div>
                ...
                <div class="file_link selected" aria-hidden="true" data-toggle="tooltip" data-placement="auto" :title="file">
                    <div class="link_icon">
                        <template v-if="fileIs(file, 'image')">
                            <div class="img_icon" :style="imgIcon('{{ Storage::disk(config('pilot.storage.disk'))->url('/') }}'+file)"></div>
                        </template>
                        <template v-else-if="fileIs(file, 'video')">
                            <i class="icon pilot-video"></i>
                        </template>
                        <template v-else-if="fileIs(file, 'audio')">
                            <i class="icon pilot-music"></i>
                        </template>
                        <template v-else-if="fileIs(file, 'zip')">
                            <i class="icon pilot-archive"></i>
                        </template>
                        <template v-else-if="fileIs(file, 'folder')">
                            <i class="icon pilot-folder"></i>
                        </template>
                        <template v-else>
                            <i class="icon pilot-file-text"></i>
                        </template>
                    </div>
                    <div class="details">
                        <div class="folder">
                    ...
                    <i class="pilot-x dd-nodrag" v-on:click="removeFileFromInput(file)"></i>
                </div>
            </li>
        </ol>
    </div>
    <div v-if="hidden_element">
        <div class="btn btn-sm btn-default" v-on:click="isExpanded = !isExpanded;" style="width:100%">
            <div v-if="!isExpanded"><i class="pilot-double-down"></i> {{ __('pilot::generic.open') }}</div>
            <div v-if="isExpanded"><i class="pilot-double-up"></i> {{ __('pilot::generic.close') }}</div>
        </div>
    </div>
    <div id="toolbar" v-if="showToolbar" :style="isExpanded ? 'display:block' : 'display:none'">
        <div class="btn-group offset-right">
            <button type="button" class="btn btn-primary" id="upload" v-if="allowUpload">
                <i class="pilot-upload"></i>
                {{ __('pilot::generic.upload') }}
            </button>
            <button type="button" class="btn btn-primary" v-if="allowCreateFolder" data-toggle="modal" :data-target="'#create_dir_modal_'+this._uid">
                <i class="pilot-folder"></i>
                {{ __('pilot::generic.add_folder') }}
            </button>
        </div>
        <button type="button" class="btn btn-default" v-on:click="getFiles()">
            <i class="pilot-refresh"></i>
        </button>
        <div class="btn-group offset-right">
            <button type="button" :disabled="selected_files.length == 0" v-if="allowUpload && hidden_element" class="btn btn-default" v-on:click="addSelectedFiles()">
                <i class="pilot-upload"></i>
                {{ __('pilot::media.add_all_selected') }}
            </button>
            <button type="button" v-if="showFolders && allowMove" class="btn btn-default" data-toggle="modal" :data-target="'#move_files_modal_'+this._uid">
                <i class="pilot-move"></i>
                {{ __('pilot::generic.move') }}
            </button>
            <button type="button" v-if="allowDelete" :disabled="selected_files.length == 0" class="btn btn-default" data-toggle="modal" :data-target="'#confirm_delete_modal_'+this._uid">
                <i class="pilot-trash"></i>
                {{ __('pilot::generic.delete') }}
            </button>
        ...
        <div class="breadcrumb-container">
            <ol class="breadcrumb filemanager">
                <li class="media_breadcrumb" v-on:click="setCurrentPath(-1)">
                    ...
                    <strong>{{ __('pilot::media.library') }}</strong>
                </li>
        ...
        <div class="flex">
            <div id="left">
                ...
                <div id="file_loader" v-if="is_loading">
                    <?php $admin_loader_img = Pilot::setting('admin.loader', ''); ?>
                    @if($admin_loader_img == '')
                    <img src="{{ pilot_asset('images/logo-icon.png') }}" alt="Pilot Loader">
                    @else
                    <img src="{{ Pilot::image($admin_loader_img) }}" alt="Pilot Loader">
                    @endif
                    <p>{{ __('pilot::media.loading') }}</p>
                </div>

                <div id="no_files" v-if="files.length == 0">
                    <h3><i class="pilot-meh"></i> {{ __('pilot::media.no_files_in_folder') }}</h3>
                </div>
            </div>
            <div id="right">
                <div class="right_details">
                    <div v-if="selected_files.length > 1" class="right_none_selected">
                        <i class="pilot-list"></i>
                        <p>@{{ selected_files.length }} {{ __('pilot::media.files_selected') }}</p>
                    </div>
                        ...
                        <div class="detail_img">
                            ...
                            <div v-else-if="fileIs(selected_file, 'video')">
                                <video width="100%" height="auto" ref="videoplayer" controls>
                                    ...
                                    {{ __('pilot::media.browser_video_support') }}
                                </video>
                            </div>
                            <div v-else-if="fileIs(selected_file, 'audio')">
                                <i class="pilot-music"></i>
                                <audio controls style="width:100%; margin-top:5px;" ref="audioplayer">
                                    ...
                                    {{ __('pilot::media.browser_audio_support') }}
                                </audio>
                            </div>
                            <div v-else-if="fileIs(selected_file, 'zip')">
                                <i class="pilot-archive"></i>
                            </div>
                            <div v-else-if="fileIs(selected_file, 'folder')">
                                <i class="pilot-folder"></i>
                            </div>
                            <div v-else>
                                <i class="pilot-file-text"></i>
                            </div>
                        </div>
                        <div class="detail_info">
                            <span>
                                <h4>{{ __('pilot::media.title') }}:</h4>
                            ...
                            <span>
                                <h4>{{ __('pilot::media.type') }}:</h4>
                                <p>@{{ selected_file.type }}</p>
                            </span>

                            <template v-if="!fileIs(selected_file, 'folder')">
                                <span>
                                    <h4>{{ __('pilot::media.size') }}:</h4>
                                    <p><span class="selected_file_size">@{{ bytesToSize(selected_file.size) }}</span></p>
                                </span>
                                <span>
                                    <h4>{{ __('pilot::media.public_url') }}:</h4>
                                    <p><a :href="selected_file.path" target="_blank">{{ __('pilot::generic.click_here') }}</a></p>
                                </span>
                                <span>
                                    <h4>{{ __('pilot::media.last_modified') }}:</h4>
                    ...
                    <div v-else class="right_none_selected">
                        <i class="pilot-cursor"></i>
                        <p>{{ __('pilot::media.nothing_selected') }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade modal-info" :id="'create_dir_modal_'+this._uid">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    <h4 class="modal-title"><i class="pilot-folder"></i> {{ __('pilot::media.add_new_folder') }}</h4>
                </div>

                <div class="modal-body">
                    <input name="new_folder_name" placeholder="{{ __('pilot::media.new_folder_name') }}" class="form-control" value="" v-model="modals.new_folder.name" />
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('pilot::generic.cancel') }}</button>
                    <button type="button" class="btn btn-info" v-on:click="createFolder">{{ __('pilot::media.create_new_folder') }}
                    </button>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade modal-danger" :id="'confirm_delete_modal_'+this._uid" v-if="allowDelete">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    <h4 class="modal-title"><i class="pilot-warning"></i> {{ __('pilot::generic.are_you_sure') }}</h4>
                </div>

                <div class="modal-body">
                    <h4>{{ __('pilot::media.delete_question') }}</h4>
                    <ul>
                        <li v-for="file in selected_files">@{{ file.name }}</li>
                    </ul>
                    <h5 class="folder_warning">
                        <i class="pilot-warning"></i> {{ __('pilot::media.delete_folder_question') }}
                    </h5>
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('pilot::generic.cancel') }}</button>
                    <button type="button" class="btn btn-danger" v-on:click="deleteFiles">{{ __('pilot::generic.delete_confirm') }}
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="pilot-move"></i> {{ __('pilot::media.move_file_folder') }}</h4>
                </div>

                <div class="modal-body">
                    <h4>{{ __('pilot::media.destination_folder') }}</h4>
                    <select class="form-control" v-model="modals.move_files.destination">
                        <option value="" disabled>{{ __('pilot::media.destination_folder') }}</option>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('pilot::generic.cancel') }}</button>
                    <button type="button" class="btn btn-warning" v-on:click="moveFiles">{{ __('pilot::generic.move') }}</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade modal-warning" :id="'crop_modal_'+this._uid" v-if="allowCrop">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    <h4 class="modal-title">{{ __('pilot::media.crop_image') }}</h4>
                </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="new-image-info">
                        {{ __('pilot::media.width') }} <span :id="'new-image-width_'+this._uid"></span>, {{ __('pilot::media.height') }}<span :id="'new-image-height_'+this._uid"></span>
                    </div>
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('pilot::generic.cancel') }}</button>
                    <button type="button" class="btn btn-warning" v-on:click="crop(false)">{{ __('pilot::media.crop') }}</button>
                    <button type="button" class="btn btn-warning" v-on:click="crop(true)">{{ __('pilot::media.crop_and_create') }}</button>
                </div>
            </div>
        </div>
    </div>
    ...
    Vue.component('media-manager', {
        ...
        methods: {
            getFiles: function() {
                var vm = this;
                vm.is_loading = true;
                $.post('{{ route('pilot.media.files') }}', { folder: vm.current_folder, _token: '{{ csrf_token() }}', details: vm.details }, function(data) {
                    vm.files = [];
                ...
                if (file.type != 'folder') {
                    if (!this.allowMultiSelect) {
                        ...
                        if (content.length >= this.maxSelectedFiles && this.maxSelectedFiles > 0) {
                            var msg_sing = "{{ trans_choice('pilot::media.max_files_select', 1) }}";
                            var msg_plur = "{{ trans_choice('pilot::media.max_files_select', 2) }}";
            ...
            renameFile: function(object) {
                ...
                $.post('{{ route('pilot.media.rename') }}', {
					...
					if (data.success == true) {
						toastr.success('{{ __('pilot::media.success_renamed') }}', "{{ __('pilot::generic.sweet_success') }}");
						vm.getFiles();
					} else {
						toastr.error(data.error, "{{ __('pilot::generic.whoopsie') }}");
					}
				});
            },
            createFolder: function(e) {
                ...
                $.post('{{ route('pilot.media.new_folder') }}', { new_folder: vm.current_folder+'/'+name, _token: '{{ csrf_token() }}' }, function(data) {
					if(data.success == true){
						toastr.success('{{ __('pilot::generic.successfully_created') }} ' + name, "{{ __('pilot::generic.sweet_success') }}");
						vm.getFiles();
					} else {
						toastr.error(data.error, "{{ __('pilot::generic.whoopsie') }}");
					}
            ...
            deleteFiles: function() {
                ...
                var vm = this;
                $.post('{{ route('pilot.media.delete') }}', {
                    path: vm.current_folder,
                    files: vm.selected_files,
                    _token: '{{ csrf_token() }}'
                }, function(data){
					if(data.success == true){
						toastr.success('', "{{ __('pilot::generic.sweet_success') }}");
						vm.getFiles();
            ...
            moveFiles: function(e) {
				...
				$.post('{{ route('pilot.media.move') }}', {
					...
					if(data.success == true){
						toastr.success('{{ __('pilot::media.success_moved') }}', "{{ __('pilot::generic.sweet_success') }}");
						vm.getFiles();
					} else {
						toastr.error(data.error, "{{ __('pilot::generic.whoopsie') }}");
					}
            ...
            crop: function(mode) {
                ...
                if (!mode) {
                    if (!window.confirm('{{ __('pilot::media.crop_override_confirm') }}')) {
						return;
					}
                }
				...
				$.post('{{ route('pilot.media.crop') }}', postData, function(data) {
					if (data.success) {
            ...
            if (this.allowUpload && !dropzone.hasClass('dz-clickable')) {
                dropzone.dropzone({
                    ...
                    success: function(e, res) {
                        if (res.success) {
                            toastr.success(res.message, "{{ __('pilot::generic.sweet_success') }}");
                        } else {
                            toastr.error(res.message, "{{ __('pilot::generic.whoopsie') }}");
                        }
                    },
                    error: function(e, res, xhr) {
                        toastr.error(res, "{{ __('pilot::generic.whoopsie') }}");
                    },
                    ...
                    if (vm.hidden_element) {
                        if (vm.maxSelectedFiles > 1) {
                            ...
                            if (content.length < vm.minSelectedFiles) {
                                e.preventDefault();
                                var msg_sing = "{{ trans_choice('pilot::media.min_files_select', 1) }}";
                                var msg_plur = "{{ trans_choice('pilot::media.min_files_select', 2) }}";
                            ...
                            if (vm.minSelectedFiles > 0 && vm.hidden_element.value == '') {
                                e.preventDefault();
                                toastr.error("{{ trans_choice('pilot::media.min_files_select', 1) }}");
                            }
                        }
                    }
                });
