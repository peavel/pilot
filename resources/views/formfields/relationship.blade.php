    ...
    @if(class_exists($options->model))
        ...
        @if($options->type == 'belongsTo')
            ...
            @if(isset($view) && ($view == 'browse' || $view == 'read'))
                ...
                @else
                    <p>{{ __('pilot::generic.no_results') }}</p>
                @endif
            ...
            @else
                ...
                <select
                    ...
                    data-get-items-route="{{route('pilot.' . $dataType->slug.'.relation')}}"
            ...
            @else
                <select
                    ...
                    @if(isset($options->taggable) && $options->taggable === 'on')
                        data-route="{{ route('pilot.'.\Illuminate\Support\Str::slug($options->table).'.store') }}"
                        data-label="{{$options->label}}"
                        data-error-message="{{__('pilot::bread.error_tagging')}}"
                    @endif
                    @if($row->required == 1) required @endif
                >
