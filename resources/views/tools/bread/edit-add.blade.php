...
@if (isset($dataType->id))
    @section('page_title', __('pilot::bread.edit_bread_for_table', ['table' => $dataType->name]))
    @php
    ...
    @endphp
@else
    @section('page_title', __('pilot::bread.create_bread_for_table', ['table' => $table]))
@endif

@section('page_header')
    <div class="page-title">
        <i class="pilot-data"></i>
        @if (isset($dataType->id))
            {{ __('pilot::bread.edit_bread_for_table', ['table' => $dataType->name]) }}
        @else
            {{ __('pilot::bread.create_bread_for_table', ['table' => $table]) }}
        @endif
    </div>
    @php
    ...
    @endphp
    @include('pilot::multilingual.language-selector')
@stop

@section('breadcrumbs')
<ol class="breadcrumb hidden-xs">
    <li class="active">
        <a href="{{ route('pilot.dashboard')}}"><i class="pilot-boat"></i> {{ __('pilot::generic.dashboard') }}</a>
    </li>
    <li class="active">
        <a href="{{ route('pilot.bread.index') }}">
            {{ __('pilot::generic.bread') }}
        </a>
    </li>
    <li class="active">
        @if(isset($dataType->id))
        <a href="{{ route('pilot.bread.edit', $table) }}">
            {{ $display_name }}
        </a>
        @else
        <a href="{{ route('pilot.bread.create', $table) }}">
            {{ $display_name }}
        </a>
        @endif
    </li>
    <li>
        {{ isset($dataType->id) ? __('pilot::generic.edit') : __('pilot::generic.add') }}
    </li>
</ol>
@endsection

@section('content')
    <div class="page-content container-fluid" id="pilotBreadEditAdd">
        <div class="row">
            <div class="col-md-12">

                <form action="@if(isset($dataType->id)){{ route('pilot.bread.update', $dataType->id) }}@else{{ route('pilot.bread.store') }}@endif"
                ...
                @endif
                    ...
                    <div class="panel panel-primary panel-bordered">
                        ...
                        <div class="panel-heading">
                            <h3 class="panel-title panel-icon"><i class="pilot-bread"></i> {{ ucfirst($table) }} {{ __('pilot::bread.bread_info') }}</h3>
                            <div class="panel-actions">
                                <a class="panel-action pilot-angle-up" data-toggle="panel-collapse" aria-hidden="true"></a>
                            </div>
                        </div>
                        ...
                        <div class="panel-body">
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="name">{{ __('pilot::database.table_name') }}</label>
                            ...
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="display_name_singular">{{ __('pilot::bread.display_name_singular') }}</label>
                                    @if($isModelTranslatable)
                                        @include('pilot::multilingual.input-hidden', [
                                    ...
                                    <input type="text" class="form-control"
                                           ...
                                           placeholder="{{ __('pilot::bread.display_name_singular') }}"
                                           value="{{ $display_name }}">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="display_name_plural">{{ __('pilot::bread.display_name_plural') }}</label>
                                    @if($isModelTranslatable)
                                        @include('pilot::multilingual.input-hidden', [
                                            'isModelTranslatable' => true,
                                    ...
                                    <input type="text" class="form-control"
                                           ...
                                           placeholder="{{ __('pilot::bread.display_name_plural') }}"
                                           value="{{ $display_name_plural }}">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="slug">{{ __('pilot::bread.url_slug') }}</label>
                                    <input type="text" class="form-control" name="slug" placeholder="{{ __('pilot::bread.url_slug_ph') }}"
                                           value="{{ $dataType->slug ?? $slug }}">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="icon">{{ __('pilot::bread.icon_hint') }} <a
                                                href="{{ route('pilot.compass.index') }}#fonts"
                                                target="_blank">{{ __('pilot::bread.icon_hint2') }}</a></label>
                                    <input type="text" class="form-control" name="icon"
                                           placeholder="{{ __('pilot::bread.icon_class') }}"
                                           value="{{ $dataType->icon ?? '' }}">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="model_name">{{ __('pilot::bread.model_name') }}</label>
                                    <span class="pilot-question"
                                        aria-hidden="true"
                                        data-toggle="tooltip"
                                        data-placement="right"
                                        title="{{ __('pilot::bread.model_name_ph') }}"></span>
                                    <input type="text" class="form-control" name="model_name" placeholder="{{ __('pilot::bread.model_class') }}"
                                           value="{{ $dataType->model_name ?? $model_name }}">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="controller">{{ __('pilot::bread.controller_name') }}</label>
                                    <span class="pilot-question"
                                        aria-hidden="true"
                                        data-toggle="tooltip"
                                        data-placement="right"
                                        title="{{ __('pilot::bread.controller_name_hint') }}"></span>
                                    <input type="text" class="form-control" name="controller" placeholder="{{ __('pilot::bread.controller_name') }}"
                                           value="{{ $dataType->controller ?? '' }}">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="policy_name">{{ __('pilot::bread.policy_name') }}</label>
                                    <span class="pilot-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('pilot::bread.policy_name_ph') }}"></span>
                                    <input type="text" class="form-control" name="policy_name" placeholder="{{ __('pilot::bread.policy_class') }}"
                                           value="{{ $dataType->policy_name ?? '' }}">
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="generate_permissions">{{ __('pilot::bread.generate_permissions') }}</label><br>
                                    ...
                                    <input type="checkbox"
                                           ...
                                           class="toggleswitch"
                                           data-on="{{ __('pilot::generic.yes') }}"
                                           data-off="{{ __('pilot::generic.no') }}"
                                           @if($checked) checked @endif >
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="server_side">{{ __('pilot::bread.server_pagination') }}</label><br>
                            ...
                            <div class="row clearfix">
                                <div class="col-md-3 form-group">
                                    <label for="order_column">{{ __('pilot::bread.order_column') }}</label>
                                    <span class="pilot-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('pilot::bread.order_column_ph') }}"></span>
                                    <select name="order_column" class="select2 form-control">
                                        <option value="">-- {{ __('pilot::generic.none') }} --</option>
                                ...
                                <div class="col-md-3 form-group">
                                    <label for="order_display_column">{{ __('pilot::bread.order_ident_column') }}</label>
                                    <span class="pilot-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('pilot::bread.order_ident_column_ph') }}"></span>
                                ...
                                <div class="col-md-3 form-group">
                                    <label for="order_direction">{{ __('pilot::bread.order_direction') }}</label>
                                    <select name="order_direction" class="select2 form-control">
                                        <option value="asc" @if(isset($dataType) && $dataType->order_direction == 'asc') selected @endif>
                                            {{ __('pilot::generic.ascending') }}
                                        </option>
                                        <option value="desc" @if(isset($dataType) && $dataType->order_direction == 'desc') selected @endif>
                                            {{ __('pilot::generic.descending') }}
                                        </option>
                                    </select>
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="default_search_key">{{ __('pilot::bread.default_search_key') }}</label>
                                    <span class="pilot-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('pilot::bread.default_search_key_ph') }}"></span>
                            ...
                            <div class="row clearfix">
                                @if (isset($scopes) && isset($dataType))
                                    <div class="col-md-3 form-group">
                                        <label for="scope">{{ __('pilot::bread.scope') }}</label>
                                ...
                                <div class="col-md-9 form-group">
                                    <label for="description">{{ __('pilot::bread.description') }}</label>
                                    <textarea class="form-control"
                                              name="description"
                                              placeholder="{{ __('pilot::bread.description') }}"
                    ...
                    <div class="panel panel-primary panel-bordered">
                        <div class="panel-heading">
                            <h3 class="panel-title panel-icon"><i class="pilot-window-list"></i> {{ __('pilot::bread.edit_rows', ['table' => $table]) }}:</h3>
                            <div class="panel-actions">
                                <a class="panel-action pilot-angle-up" data-toggle="panel-collapse" aria-hidden="true"></a>
                            </div>
                        </div>
                        ...
                        <div class="panel-body">
                            <div class="row fake-table-hd">
                                <div class="col-xs-2">{{ __('pilot::database.field') }}</div>
                                <div class="col-xs-2">{{ __('pilot::database.visibility') }}</div>
                                <div class="col-xs-2">{{ __('pilot::database.input_type') }}</div>
                                <div class="col-xs-2">{{ __('pilot::bread.display_name') }}</div>
                                <div class="col-xs-4">{{ __('pilot::database.optional_details') }}</div>
                            </div>
                            ...
                            @foreach($fieldOptions as $data)
                                ...
                                @if(isset($dataType->id))
                                    <?php $dataRow = Pilot::model('DataRow')->where('data_type_id', '=', $dataType->id)->where('field', '=', $data['field'])->first(); ?>
                                @endif

                                <div class="row row-dd">
                                    <div class="col-xs-2">
                                        <h4><strong>{{ $data['field'] }}</strong></h4>
                                        <strong>{{ __('pilot::database.type') }}:</strong> <span>{{ $data['type'] }}</span><br/>
                                        <strong>{{ __('pilot::database.key') }}:</strong> <span>{{ $data['key'] }}</span><br/>
                                        <strong>{{ __('pilot::generic.required') }}:</strong>
                                        @if($data['null'] == "NO")
                                            <span>{{ __('pilot::generic.yes') }}</span>
                                            <input type="hidden" value="1" name="field_required_{{ $data['field'] }}" checked="checked">
                                        @else
                                            <span>{{ __('pilot::generic.no') }}</span>
                                            <input type="hidden" value="0" name="field_required_{{ $data['field'] }}">
                                        @endif
                                        <div class="handler pilot-handle"></div>
                                    ...
                                    <div class="col-xs-2">
                                        <input type="checkbox"
                                               ...
                                               @elseif(!isset($dataRow->browse))
                                                   checked="checked"
                                               @endif>
                                        <label for="field_browse_{{ $data['field'] }}">{{ __('pilot::generic.browse') }}</label><br/>
                                        <input type="checkbox"
                                        ...
                                        <label for="field_read_{{ $data['field'] }}">{{ __('pilot::generic.read') }}</label><br/>
                                        <input type="checkbox"
                                        ...
                                        <label for="field_edit_{{ $data['field'] }}">{{ __('pilot::generic.edit') }}</label><br/>
                                        <input type="checkbox"
                                            ...
                                            <label for="field_add_{{ $data['field'] }}">{{ __('pilot::generic.add') }}</label><br/>
                                        <input type="checkbox"
                                    ...
                                    <div class="col-xs-2">
                                        @if($isModelTranslatable)
                                            @include('pilot::multilingual.input-hidden', [
                                                ...
                                                '_field_trans' => $dataRow ? get_field_translations($dataRow, 'display_name') : json_encode([config('pilot.multilingual.default') => ucwords(str_replace('_', ' ', $data['field']))]),
                                            ])
                                        @endif
                                        <input type="text" class="form-control"
                            ...
                            @if(isset($dataTypeRelationships))
                                @foreach($dataTypeRelationships as $relationship)
                                    @include('pilot::tools.bread.relationship-partial', $relationship)
                                @endforeach
                            @endif
                        ...
                        <div class="panel-footer">
                             <div class="btn btn-new-relationship"><i class="pilot-heart"></i> <span>
                             {{ __('pilot::database.relationship.create') }}</span></div>
                        </div>
                    </div><!-- .panel -->

                    <button type="submit" class="btn pull-right btn-primary">{{ __('pilot::generic.submit') }}</button>
...
@include('pilot::tools.bread.relationship-new-modal')
...
@section('javascript')
    ...
    <script>
            ...
            $('textarea[data-editor]').each(function () {
                ...
                textarea.closest('form').on('submit', function (ev) {
                    if (window.invalidEditors.length) {
                        ...
                        toastr.error('{{ __('pilot::json.invalid_message') }}', '{{ __('pilot::json.validation_errors') }}', {"preventDuplicates": true, "preventOpenDuplicates": true});
                    } else {
        ...
        $(function () {
            ...
            $('.pilot-relationship-details-btn').click(function(){
                $(this).toggleClass('open');
                if($(this).hasClass('open')){
                    $(this).parent().parent().find('.pilot-relationship-details').slideDown();
                    populateRowsFromTable($(this).parent().parent().find('select.relationship_table'));
                } else {
                    $(this).parent().parent().find('.pilot-relationship-details').slideUp();
                }
            });
        ...
        function populateRowsFromTable(dropdown){
            ...
            $.get('{{ route('pilot.database.index') }}/' + tbl, function(data){
