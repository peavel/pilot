...
<tr class="newTableRow">
    ...
    <td>
        <select :value="index.type" @change="onIndexTypeChange"
                ...
                class="form-control">
            <option value=""></option>
            <option value="INDEX">{{ __('pilot::database.index') }}</option>
            <option value="UNIQUE">{{ __('pilot::database.unique') }}</option>
            <option value="PRIMARY">{{ __('pilot::database.primary') }}</option>
        </select>
        <small v-if="column.composite" v-once>{{ __('pilot::database.composite_warning') }}</small>
    </td>
    ...
    <td>
        <div class="btn btn-danger delete-row" @click="deleteColumn"><i class="pilot-trash"></i></div>
    </td>
...
@include('pilot::tools.database.vue-components.database-types')
@include('pilot::tools.database.vue-components.database-column-default')
    ...
    Vue.component('database-column', {
        ...
        methods: {
            ...
            onIndexTypeChange(event) {
                if (this.column.name == '') {
                    return toastr.error("{{ __('pilot::database.name_warning') }}");
                }
