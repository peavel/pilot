...
class CompassTest extends TestCase
{
    ...
    {
        $this->app['config']->set('pilot.compass_in_production', true);
    }
    ...
    public function testCantAccessCompass()
    {
        ...
        $response = $this->call('GET', route('pilot.compass.index'));
        $this->assertEquals(403, $response->status());
    }

    public function testCanAccessCompass()
    {
        // Can access compass because we set pilot.compass_in_production configuration to true
        $this->enableCompass();
    ...
    public function testCanSeeLaravelLog()
    {
        ...
        $this->visit(route('pilot.compass.index').'?log='.base64_encode('laravel.log'))
             ->see($info);
    }
    ...
    public function testCanExecuteCommand()
    {
        ...
        $response = $this->post(route('pilot.compass.index'), [
            'command' => 'make:model',
            'args'    => 'TestModel',
        ]);
    ...
    public function testCanDeleteLaravelLog()
    {
        ...
        $response = $this->call('GET', route('pilot.compass.index').'?del='.base64_encode('laravel.log'));
