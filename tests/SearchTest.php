...
use PEAVEL\Pilot\Http\Controllers\PilotBaseController;
...
class SearchTest extends TestCase
{
    ...
    public function testCanSearchEquals0()
    {
        ...
        $this->visit(route('pilot.posts.index').'?'.http_build_query($params))
            ->dontSee($post->title)
    ...
    public function testCanSearchByBelongstoRelationship()
    {
        ...
        $response = $this->fakeVisit('pilot.posts.index', 'GET', $params);
    ...
    protected function fakeVisit($route, $method = 'GET', $params = [])
    {
        ...
        return (new PilotBaseController())->index($request);
    }
    ...
    protected function setupAuthorRelationship()
    {
        DataRow::create([
            ...
            'display_name' => 'Author',
            'details' => [
                'model' => 'PEAVEL\Pilot\Models\User',
                'table' => 'users',
                'type' => 'belongsTo',
                'column' => 'author_id',
                'key' => 'id',
