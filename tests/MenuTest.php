...
class MenuTest extends TestCase
{
    ...
    public function testCanRenameMenu()
    {
        ...
        $this->visitRoute('pilot.menus.edit', $menu->id)
             ->seeInField('name', $menu->name)
             ->type('new_admin', 'name')
             ->seeInElement('button', __('pilot::generic.save'))
             ->press(__('pilot::generic.save'))
             ->seePageIs(route('pilot.menus.index'))
             ->seeInDatabase('menus', [
                 'id'   => $menu->id,
                 'name' => 'new_admin',
             ]);
    ...
    {
        $menu = Menu::where('name', '=', 'admin')->first();
        $this->delete(route('pilot.menus.item.destroy', [
            'menu' => $menu->id,
    ...
    {
        $menu = Menu::where('name', '=', 'admin')->first();
        $this->post(route('pilot.menus.item.add', [
            'menu'    => $menu->id,
            'menu_id' => $menu->id,
            'type'    => 'url',
            'color'   => '#000000',
    ...
    public function testCanUpdateMenuItem()
    {
        ...
        $this->put(route('pilot.menus.item.update', [
            'id'      => $item->id,
            'menu'    => $menu->id,
            'type'    => 'url',
            'color'   => '#000000',
    ...
    {
        $menu = Menu::where('name', '=', 'admin')->first();
        $response = $this->call('GET', route('pilot.menus.builder', ['menu' => $menu->id]));
