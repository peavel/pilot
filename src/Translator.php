...
class Translator implements ArrayAccess, JsonSerializable
{
    ...
    public function __construct(Model $model)
    {
        ...
        $this->model = $model;
        $this->locale = config('pilot.multilingual.default', 'en');
        $attributes = [];
    ...
    public function save()
    {
        ...
        foreach ($attributes as $key => $attribute) {
            if ($attribute['exists']) {
                ...
                $translation = PilotFacade::model('Translation')->where('table_name', $this->model->getTable())
            ...
            if (is_null($translation)) {
                $translation = PilotFacade::model('Translation');
            }
    ...
    protected function translateAttributeToOriginal($attribute)
    {
        $this->attributes[$attribute] = [
            ...
            'locale'   => config('pilot.multilingual.default', 'en'),
            'exists'   => true,
            'modified' => false,
        ];
    ...
    public function deleteTranslation($key)
    {
        ...
        PilotFacade::model('Translation')->where('table_name', $this->model->getTable())
            ->where('column_name', $key)
            ->where('foreign_key', $this->model->getKey())
            ->where('locale', $locale)
            ->delete();
    ...
    public function __call($method, array $arguments)
    {
        if (!$this->model->hasTranslatorMethod($method)) {
            throw new \Exception('Call to undefined method PEAVEL\Pilot\Translator::'.$method.'()');
        }
