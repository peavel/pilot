...
use PEAVEL\Pilot\Events\MenuDisplay;
...
class Menu extends Model
{
    ...
    {
        return $this->hasMany(Pilot::modelClass('MenuItem'));
    }
    ...
    public static function display($menuName, $type = null, array $options = [])
    {
        ...
        $menu = \Cache::remember('pilot_menu_'.$menuName, \Carbon\Carbon::now()->addDays(30), function () use ($menuName) {
        ...
        if ($type == 'admin') {
            $type = 'pilot::menu.'.$type;
        } else {
            if (is_null($type)) {
                $type = 'pilot::menu.default';
            } elseif ($type == 'bootstrap' && !view()->exists($type)) {
                $type = 'pilot::menu.bootstrap';
            }
        }
    ...
    {
        \Cache::forget('pilot_menu_'.$this->name);
    }
    ...
    protected static function processItems($items)
    {
        ...
        $items = $items->transform(function ($item) {
            ...
            if (($item->href == url('') || $item->href == route('pilot.dashboard')) && $item->children->count() > 0) {
                // Exclude sub-menus
                $item->active = false;
            } elseif ($item->href == route('pilot.dashboard') && url()->current() != route('pilot.dashboard')) {
                // Exclude dashboard
                $item->active = false;
            }
