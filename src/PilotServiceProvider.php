...
use PEAVEL\Pilot\Events\FormFieldsRegistered;
use PEAVEL\Pilot\Facades\Pilot as PilotFacade;
use PEAVEL\Pilot\FormFields\After\DescriptionHandler;
use PEAVEL\Pilot\Http\Middleware\PilotAdminMiddleware;
use PEAVEL\Pilot\Models\MenuItem;
use PEAVEL\Pilot\Models\Setting;
use PEAVEL\Pilot\Policies\BasePolicy;
use PEAVEL\Pilot\Policies\MenuItemPolicy;
use PEAVEL\Pilot\Policies\SettingPolicy;
use PEAVEL\Pilot\Providers\PilotDummyServiceProvider;
use PEAVEL\Pilot\Providers\PilotEventServiceProvider;
use PEAVEL\Pilot\Seed;
use PEAVEL\Pilot\Translator\Collection as TranslatorCollection;
...
class PilotServiceProvider extends ServiceProvider
{
    ...
    {
        $this->app->register(PilotEventServiceProvider::class);
        $this->app->register(ImageServiceProvider::class);
        $this->app->register(PilotDummyServiceProvider::class);
        $this->app->register(DoctrineSupportServiceProvider::class);

        $loader = AliasLoader::getInstance();
        $loader->alias('Pilot', PilotFacade::class);

        $this->app->singleton('pilot', function () {
            return new Pilot();
        });

        $this->app->singleton('PilotGuard', function () {
    ...
    {
        if (config('pilot.user.add_default_role_on_register')) {
            $model = Auth::guard(app('PilotGuard'))->getProvider()->getModel();
            call_user_func($model.'::created', function ($user) use ($model) {
                if (is_null($user->role_id)) {
                    call_user_func($model.'::findOrFail', $user->id)
                        ->setRole(config('pilot.user.default_role'))
                        ->save();
                }
            });
        }
        ...
        $router->aliasMiddleware('admin.user', PilotAdminMiddleware::class);

        $this->loadTranslationsFrom(realpath(__DIR__.'/../publishable/lang'), 'pilot');

        if (config('pilot.database.autoload_migrations', true)) {
        ...
        $event->listen('pilot.alerts.collecting', function () {
    ...
    {
        // Register alerts
        View::composer('pilot::*', function ($view) {
            $view->with('alerts', PilotFacade::alerts());
        });
    }
    ...
    protected function addStorageSymlinkAlert()
    {
        ...
        if ($routeName != 'pilot.dashboard') {
            return;
        }

        $storage_disk = (!empty(config('pilot.storage.disk'))) ? config('pilot.storage.disk') : 'public';
        ...
        if (request()->has('fix-missing-storage-symlink')) {
            ...
            if (!file_exists(public_path('storage')) || @readlink(public_path('storage')) == public_path('storage')) {
                $alert = (new Alert('missing-storage-symlink', 'warning'))
                    ->title(__('pilot::error.symlink_missing_title'))
                    ->text(__('pilot::error.symlink_missing_text'))
                    ->button(__('pilot::error.symlink_missing_button'), '?fix-missing-storage-symlink=1');
                PilotFacade::addAlert($alert);
            }
        }
    }
    ...
    protected function fixMissingStorageSymlink()
    {
        ...
        if (file_exists(public_path('storage'))) {
            $alert = (new Alert('fixed-missing-storage-symlink', 'success'))
                ->title(__('pilot::error.symlink_created_title'))
                ->text(__('pilot::error.symlink_created_text'));
        } else {
            $alert = (new Alert('failed-fixing-missing-storage-symlink', 'danger'))
                ->title(__('pilot::error.symlink_failed_title'))
                ->text(__('pilot::error.symlink_failed_text'));
        }
    ...
    protected function registerAlertComponents()
    {
        ...
        foreach ($components as $component) {
            $class = 'PEAVEL\\Pilot\\Alert\\Components\\'.ucfirst(Str::camel($component)).'Component';

            $this->app->bind("pilot.alert.components.{$component}", $class);
        }
    }
    ...
    private function registerPublishableResources()
    {
        ...
        $publishable = [
            ...
            'config' => [
                "{$publishablePath}/config/pilot.php" => config_path('pilot.php'),
            ],
    ...
    public function registerConfigs()
    {
        $this->mergeConfigFrom(
            dirname(__DIR__).'/publishable/config/pilot.php',
            'pilot'
        );
    }
    ...
    public function loadAuth()
    {
        ...
        try {
            if (Schema::hasTable(PilotFacade::model('DataType')->getTable())) {
                $dataType = PilotFacade::model('DataType');
            ...
            Log::info('No database connection yet in PilotServiceProvider loadAuth(). No worries, this is not a problem!');
        }
    ...
    protected function registerFormFields()
    {
        ...
        foreach ($formFields as $formField) {
            ...
            PilotFacade::addFormField("PEAVEL\\Pilot\\FormFields\\{$class}");
        }

        PilotFacade::addAfterFormField(DescriptionHandler::class);
