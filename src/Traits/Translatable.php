...
trait Translatable
{
    ...
    {
        return $this->hasMany(Pilot::model('Translation'), 'foreign_key', $this->getKeyName())
            ->where('table_name', $this->getTable())
            ->whereIn('locale', config('pilot.multilingual.locales', []));
    }
    ...
    public function getTranslationsOf($attribute, array $languages = null, $fallback = true)
    {
        if (is_null($languages)) {
            $languages = config('pilot.multilingual.locales', [config('pilot.multilingual.default')]);
        }
    ...
    public function getTranslatedAttributeMeta($attribute, $locale = null, $fallback = true)
    {
        ...
        if (!in_array($attribute, $this->getTranslatableAttributes())) {
            return [$this->getAttribute($attribute), config('pilot.multilingual.default'), false];
        }
        ...
        $default = config('pilot.multilingual.default');
    ...
    public function setAttributeTranslations($attribute, array $translations, $save = false)
    {
        ...
        $default = config('pilot.multilingual.default', 'en');
        $locales = config('pilot.multilingual.locales', [$default]);
    ...
    public function prepareTranslations($request)
    {
        ...
        foreach ($fields as $field) {
            ...
            $request->merge([$field => $trans[config('pilot.multilingual.default', 'en')]]);
    ...
    public function prepareTranslationsFromArray($field, &$requestData)
    {
        ...
        // Set the default local value
        $requestData['display_name'] = $trans[config('pilot.multilingual.default', 'en')];
