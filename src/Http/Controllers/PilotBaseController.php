...
use PEAVEL\Pilot\Events\BreadDataRestored;
use PEAVEL\Pilot\Events\BreadDataUpdated;
use PEAVEL\Pilot\Events\BreadImagesDeleted;
use PEAVEL\Pilot\Facades\Pilot;
use PEAVEL\Pilot\Http\Controllers\Traits\BreadRelationshipParser;

class PilotBaseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!empty($dataTypeContent->first())) {
            foreach (Pilot::actions() as $action) {
        ...
        $view = 'pilot::bread.browse';

        if (view()->exists("pilot::$slug.browse")) {
            $view = "pilot::$slug.browse";
        }

        return Pilot::view($view, compact(
            'actions',
            'dataType',
            'dataTypeContent',
            'isModelTranslatable',
    ...
    public function show(Request $request, $id)
    {
        ...
        $view = 'pilot::bread.read';

        if (view()->exists("pilot::$slug.read")) {
            $view = "pilot::$slug.read";
        }

        return Pilot::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        $view = 'pilot::bread.edit-add';

        if (view()->exists("pilot::$slug.edit-add")) {
            $view = "pilot::$slug.edit-add";
        }

        return Pilot::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (auth()->user()->can('browse', app($dataType->model_name))) {
            $redirect = redirect()->route("pilot.{$dataType->slug}.index");
        } else {
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $data = $res
            ? [
                'message'    => __('pilot::generic.successfully_deleted')." {$displayName}",
                'alert-type' => 'success',
            ]
            : [
                'message'    => __('pilot::generic.error_deleting')." {$displayName}",
                'alert-type' => 'error',
            ];
        ...
        return redirect()->route("pilot.{$dataType->slug}.index")->with($data);
    }
    ...
    public function restore(Request $request, $id)
    {
        ...
        $data = $res
            ? [
                'message'    => __('pilot::generic.successfully_restored')." {$displayName}",
                'alert-type' => 'success',
            ]
            : [
                'message'    => __('pilot::generic.error_restoring')." {$displayName}",
                'alert-type' => 'error',
            ];
    ...
    public function remove_media(Request $request)
    {
        try {
            ...
            if (!isset($data->{$field})) {
                throw new Exception(__('pilot::generic.field_does_not_exist'), 400);
            }
            ...
            if (@json_decode($multi)) {
                ...
                if (is_null(@json_decode($data->{$field}))) {
                    throw new Exception(__('pilot::json.invalid'), 500);
                }
                ...
                if (is_null($key) || $key === false) {
                    throw new Exception(__('pilot::media.file_does_not_exist'), 400);
                }
            ...
            return response()->json([
                'data' => [
                    'status'  => 200,
                    'message' => __('pilot::media.file_removed'),
                ],
            ]);
        } catch (Exception $e) {
            $code = 500;
            $message = __('pilot::generic.internal_error');
    ...
    public function deleteBreadImages($data, $rows, $single_image = null)
    {
        ...
        foreach ($rows as $row) {
            ...
            foreach ($images_to_remove as $image) {
                ...
                if ($image != config('pilot.user.default_avatar') && (is_null($single_image) || $single_image == $image)) {
    ...
    public function order(Request $request)
    {
        ...
        if (empty($dataType->order_column) || empty($dataType->order_display_column)) {
            ...
            ->with([
                'message'    => __('pilot::bread.ordering_not_set'),
                'alert-type' => 'error',
            ]);
        }
        ...
        $dataRow = Pilot::model('DataRow')->whereDataTypeId($dataType->id)->whereField($display_column)->first();

        $view = 'pilot::bread.order';

        if (view()->exists("pilot::$slug.order")) {
            $view = "pilot::$slug.order";
        }
