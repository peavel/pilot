...
use ReflectionClass;
use PEAVEL\Pilot\Database\Schema\SchemaManager;
use PEAVEL\Pilot\Database\Schema\Table;
...
class PilotBreadController extends Controller
{
    public function index()
    {
        $this->authorize('browse_bread');

        $dataTypes = Pilot::model('DataType')->select('id', 'name', 'slug')->get()->keyBy('name')->toArray();
        ...
        return Pilot::view('pilot::tools.bread.index')->with(compact('dataTypes', 'tables'));
    }
    ...
    public function create(Request $request, $table)
    {
        ...
        $dataType = Pilot::model('DataType')->whereName($table)->first();
        ...
        return Pilot::view('pilot::tools.bread.edit-add', $data);
    }

    private function prepopulateBreadInfo($table)
    {
        $displayName = Str::singular(implode(' ', explode('_', Str::title($table))));
        $modelNamespace = config('pilot.models.namespace', app()->getNamespace());
    ...
    public function store(Request $request)
    {
        ...
        try {
            $dataType = Pilot::model('DataType');
            $res = $dataType->updateDataType($request->all(), true);
            $data = $res
                ? $this->alertSuccess(__('pilot::bread.success_created_bread'))
                : $this->alertError(__('pilot::bread.error_creating_bread'));
            if ($res) {
                event(new BreadAdded($dataType, $data));
            }

            return redirect()->route('pilot.bread.index')->with($data);
        } catch (Exception $e) {
            return redirect()->route('pilot.bread.index')->with($this->alertException($e, 'Saving Failed'));
        }
    }
    ...
    public function edit($table)
    {
        ...
        $dataTypeRelationships = Pilot::model('DataRow')->where('data_type_id', '=', $dataType->id)->where('type', '=', 'relationship')->get();
        $scopes = [];
        if ($dataType->model_name != '') {
            $scopes = $this->getModelScopes($dataType->model_name);
        }

        return Pilot::view('pilot::tools.bread.edit-add', compact('dataType', 'fieldOptions', 'isModelTranslatable', 'tables', 'dataTypeRelationships', 'scopes'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        /* @var \PEAVEL\Pilot\Models\DataType $dataType */
        try {
            $dataType = Pilot::model('DataType')->find($id);
            ...
            $res = $dataType->updateDataType($request->all(), true);
            $data = $res
                ? $this->alertSuccess(__('pilot::bread.success_update_bread', ['datatype' => $dataType->name]))
                : $this->alertError(__('pilot::bread.error_updating_bread'));
            if ($res) {
            ...
            return back()->with($this->alertException($e, __('pilot::generic.update_failed')));
        }
    }
    ...
    public function destroy($id)
    {
        ...
        $res = Pilot::model('DataType')->destroy($id);
        $data = $res
            ? $this->alertSuccess(__('pilot::bread.success_remove_bread', ['datatype' => $dataType->name]))
            : $this->alertError(__('pilot::bread.error_updating_bread'));
        if ($res) {
        ...
        if (!is_null($dataType)) {
            Pilot::model('Permission')->removeFrom($dataType->name);
        }
    ...
    public function addRelationship(Request $request)
    {
        ...
        try {
            ...
            $className = Pilot::modelClass('DataRow');
            ...
            $newRow->order = intval(Pilot::model('DataType')->find($request->data_type_id)->lastRow()->order) + 1;
    ...
    private function getRelationshipField($request)
    {
        ...
        $dataType = Pilot::model('DataType')->find($request->data_type_id);
        ...
        $existingRow = Pilot::model('DataRow')->where('field', '=', $relationshipField)->first();
        $index = 1;
    ...
    {
        Pilot::model('DataRow')->destroy($id);
