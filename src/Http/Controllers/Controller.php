...
use PEAVEL\Pilot\Http\Controllers\ContentTypes\Checkbox;
use PEAVEL\Pilot\Http\Controllers\ContentTypes\Coordinates;
use PEAVEL\Pilot\Http\Controllers\ContentTypes\File;
use PEAVEL\Pilot\Http\Controllers\ContentTypes\Image as ContentImage;
use PEAVEL\Pilot\Http\Controllers\ContentTypes\MultipleCheckbox;
use PEAVEL\Pilot\Http\Controllers\ContentTypes\MultipleImage;
use PEAVEL\Pilot\Http\Controllers\ContentTypes\Password;
use PEAVEL\Pilot\Http\Controllers\ContentTypes\Relationship;
use PEAVEL\Pilot\Http\Controllers\ContentTypes\SelectMultiple;
use PEAVEL\Pilot\Http\Controllers\ContentTypes\Text;
use PEAVEL\Pilot\Http\Controllers\ContentTypes\Timestamp;
use PEAVEL\Pilot\Traits\AlertsMessages;
use Validator;
...
abstract class Controller extends BaseController
{
    ...
    public function insertUpdateData($request, $slug, $rows, $data)
    {
        ...
        if ($request->session()->has($slug.'_path') || $request->session()->has($slug.'_uuid')) {
            ...
            if ($old_path != $new_path && 
                !Storage::disk(config('pilot.storage.disk'))->exists($new_path) && 
                Storage::disk(config('pilot.storage.disk'))->exists($old_path)
                ) 
            {
                $request->session()->forget([$slug.'_path', $slug.'_uuid']);
                Storage::disk(config('pilot.storage.disk'))->move($old_path, $new_path);
                Storage::disk(config('pilot.storage.disk'))->deleteDirectory($folder_path);
            }
        }
    ...
    {
        if (Storage::disk(config('pilot.storage.disk'))->exists($path)) {
            Storage::disk(config('pilot.storage.disk'))->delete($path);
