...
use PEAVEL\Pilot\Database\DatabaseUpdater;
use PEAVEL\Pilot\Database\Schema\Column;
use PEAVEL\Pilot\Database\Schema\Identifier;
...
class PilotDatabaseController extends Controller
{
    public function index()
    {
        ...
        return Pilot::view('pilot::tools.database.index')->with(compact('dataTypes', 'tables'));
    }
    ...
    public function create()
    {
        ...
        return Pilot::view('pilot::tools.database.edit-add', compact('db'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (isset($request->create_model) && $request->create_model == 'on') {
                ...
                Artisan::call('pilot:make:model', $params);
            ...
            return redirect()
               ...
               ->with($this->alertSuccess(__('pilot::database.success_create_table', ['table' => $table->name])));
        } catch (Exception $e) {
    ...
    public function edit($table)
    {
        ...
        if (!SchemaManager::tableExists($table)) {
            return redirect()
                ->route('pilot.database.index')
                ->with($this->alertError(__('pilot::database.edit_table_not_exist')));
        }
    ...
    protected function prepareDbManager($action, $table = '')
    {
        ...
        if ($action == 'update') {
            ...
            $db->formAction = route('pilot.database.update', $table);
        } else {
            ...
            $db->formAction = route('pilot.database.store');
        }
    ...
    public function show($table)
    {
        ...
        $model_name = Pilot::model('DataType')->where('name', $table)->pluck('model_name')->first();
        if (isset($model_name)) {
    ...
    public function destroy($table)
    {
        ...
        try {
            ...
            return redirect()
                ->route('pilot.database.index')
                ->with($this->alertSuccess(__('pilot::database.success_delete_table', ['table' => $table])));
        } catch (Exception $e) {
